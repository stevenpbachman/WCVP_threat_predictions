...
family_size <- 3500
distributions <- read_delim("01_raw_data/wcvp_distribution.txt",
             delim="|", escape_double=FALSE, trim_ws=TRUE) |>
  filter(introduced + extinct + location_doubtful == 0) |>
  filter(! is.na(area_code_l3))
names <- read_delim("01_raw_data/wcvp_names.txt",
...
home_dir <- getwd()
model_dir <- file.path(home_dir,"output/submitted/")
...
#species_zones$plant_name_id <- as.character(species_zones$plant_name_id) 

zone_perf <- test_preds |>
    dplyr::select(cv, plant_name_id, obs, .pred_class) |>
...
ggsave("05_results/submitted/bart-performance-disaggregated-climate.svg", perf_plots_climate, height=10, width=7)
...
perf_plots_zone <- (((lifeform_perf_plot + labs(x="")) / 
 ...
 plot_layout(guides="collect", heights=c(1, 1), ) +
  ...
  theme(legend.position="bottom", 
        ...
        text=element_text(family="DejaVu Sans")) &
  guides(fill="none", colour=guide_legend(title=""))
ggsave("05_results/submitted/bart-performance-disaggregated-zones.svg", perf_plots_zone, height=10, width=7)
...
ggsave("05_results/submitted/bart-performance-maps-vertical.svg", vertical,
       height=9, width=6)
...
ggsave("05_results/submitted/bart-feature-importance-full.svg", full_importance_plot, height=10, width=6)
...
ggsave("05_results/submitted/bart-feature-importance-grouped.svg", grouped_importance_plot, height=3, width=6)
...
threatened_size <-
  distributions |>
    tibble() |>
    inner_join(pred_data |> filter(obs == "threatened") |> dplyr::select(plant_name_id, L3_count), by="plant_name_id") |>
...
ggsave("05_results/submitted/bart-senitivity-range.svg", sensitivity_vs_range, height=3, width=6)
...
ggsave("05_results/submitted/bart-criteria-performance.svg", criteria_plot, height=3, width=6)
...
ggsave("05_results/submitted/bart-b-criterion-performance.svg", b_criteria_plot, height=3, width=6)
...
ggsave("05_results/submitted/bart-threat-comparison.svg", threat_comparison, height=3, width=6)
...
ggsave("05_results/submitted/bart-predictions-disaggregated-climate.svg", pred_plot_climate, height=10, width=7)
...
ggsave("05_results/submitted/bart-predictions-disaggregated-zones.svg", pred_plot_zones, height=10, width=7)
...
ggsave("05_results/submitted/bart-prediction-map.svg", pred_map, height=5, width=10)
...
), "05_results/submitted/bart-summary.xlsx")
...
write_csv(species_probs, "05_results/submitted/bart-species-probabilities.csv")
